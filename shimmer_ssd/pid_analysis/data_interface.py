...
# Model-related functions will be imported lazily when needed
...
class ModelDataProvider(DataProvider):
    ...
    def get_data(self, n_samples: int, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        # ========================================
        # CRITICAL: Ensure deterministic data generation for consistent cache hashes
        import torch
        import numpy as np
        import random
        import os
    ...
    def get_model(self):
        """
        Get the loaded model. Loads it if not already loaded.
        
        Returns:
            The loaded model with gw_encoders, or None if loading failed
        """
        if self.model is None:
            try:
